classdef StaticKinet < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        XUpperLimnmEditField3_2         matlab.ui.control.NumericEditField
    ...
    methods (Access = private)
        ...
        function MainParameterCheck(app)
            ...
            if strcmp(app.NumberofComplexesSwitch.Value, '2')
                app.EasyAlignCheckBox.Visible = "on";
            else
                app.EasyAlignCheckBox.Visible = "off";
            end
        ...
        function KinetochoreValueChangeCheck(app)
            ...
            if app.NumberofArmsSpinner.Value < app.NumberofBoundArmsSpinner.Value % Check to see if the user requested more bound complexes than complexes to begin with
                app.NumberofBoundArmsSpinner.Value = app.NumberofArmsSpinner.Value; % If so, set the number of bound complexes to the number of complexes in general
            end
        ...
        function SaveBioDistanceData(app)
            ...
            if (app.EasyAlignCheckBox.Value == true && strcmp(app.NumberofComplexesSwitch.Value, '2'))
        ...
        function UpdateRandomVariables(app)
            ...
            if app.RotationCheckBox.Value == true % Check to see if rotation is desired
                app.Rotation = rand(app.NumberofMicrotubulesSpinner.Value*NoC,3); % If so, generate an array that is 3 by the total number of chromosomes needed
            ...
            if ~strcmp(app.StaggerDropDown.Value, 'None') % Check to see if a stagger is desired
                if strcmp(app.StaggerDropDown.Value, 'Random') % If so, check to see what kind of stagger is desired
                    app.Stagger = (rand(1,app.NumberofMicrotubulesSpinner.Value*NoC)-0.5)*(app.RangenmSpinner.Value*2); % If random stagger is desired, create a vector that is as long as the total number of chromosomes that range from -1 to 1. Multiply that by the range to get the random stagger
                else
                    app.Stagger = (app.LengthSim(ceil(rand(1,app.NumberofMicrotubulesSpinner.Value*NoC)*size(app.LengthSim,1)),ceil(rand(1,app.NumberofMicrotubulesSpinner.Value*NoC)*size(app.LengthSim,2)))-3.5e-07)*1e9; % If set stagger, create a vector of stagger values samples from LengthSim that is as long as the total number of chromosomes
                end
            end
        end
        ...
        function N_Nuf2Structure2(app)
            ...
            app.N_Nuf2PointInfo.CoordPoints2 = StructurePointsGenerate(app,1+app.NumberofMicrotubulesSpinner.Value,app.NumberofMicrotubulesSpinner.Value,app.StaggerDropDown.Value,app.ComplexDiameternmSpinner.Value,app.N_Nuf2NumberofFluorophoreSpinner.Value,app.N_Nuf2LengthnmSpinner.Value,app.MicrotubuleDiameternmSpinner.Value,RotationCheck);
    ...
    methods (Access = private)
        ...
        % Callback function: ComplexDiameternmSpinner, 
        % ComplexDiameternmSpinner, LowLimSpinner, LowLimSpinner, 
        % MicrotubuleDiameternmSpinner, MicrotubuleDiameternmSpinner, 
        % NumberofComplexesSwitch, NumberofMicrotubulesSpinner, 
        % NumberofMicrotubulesSpinner, RangenmSpinner, 
        % RangenmSpinner, RotationCheckBox, RotationParametersPanel, 
        % StaggerDropDown, UpLimSpinner, UpLimSpinner, XSpinner, 
        % XSpinner, YSpinner, YSpinner, ZSpinner, ZSpinner
        ...
        % NumberofArmsSpinner, NumberofArmsSpinner, 
        % NumberofBoundArmsSpinner, NumberofBoundArmsSpinner, 
        % PointMarkedSpinner, PointMarkedSpinner, UpperAngleSpinner, 
        % UpperAngleSpinner
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.N_Nuf2NumberofFluorophoreSpinnerLabel.Position = [26 38 134 22];
            ...
            app.N_Nuf2LengthnmSpinnerLabel.Position = [26 59 134 22];
            ...
            app.N_Nuf2ColorChannelDropDownLabel.Position = [26 13 134 22];
            ...
            app.FileFolderNameStringLabel.Tooltip = {'The name of the structure. This will be used as the base string for all of the folders and files that are generated.'};
            ...
            app.GenerateXMLCheckBox.Tooltip = {'Save 3D structure as XML for Microscope Simulator'};
            ...
            app.SaveMATFileCheckBox.Tooltip = {'Save 3D structure as a MAT file.'};
            ...
            app.GainLabel.Tooltip = {'Gain parameter for simulated image generation using Microscope Simulator.'};
            ...
            app.OffsetLabel.Tooltip = {'Offset parameter for simulated image generation using Microscope Simulator.'};
            ...
            app.GaussianNoiseSDLabel.Tooltip = {'Gaussian noise added to simulated image generation using Microscope Simulator in standard deviation.'};
            ...
            app.MaxVoxelIntensityLabel.Tooltip = {'Max. Voxel Intensity parameter for simulated image generation using Microscope Simulator.'};
            ...
            % Create XUpperLimnmEditField3_2
            app.XUpperLimnmEditField3_2 = uieditfield(app.AxisLimPanel, 'numeric');
            app.XUpperLimnmEditField3_2.LowerLimitInclusive = 'off';
            app.XUpperLimnmEditField3_2.UpperLimitInclusive = 'off';
            app.XUpperLimnmEditField3_2.HorizontalAlignment = 'center';
            app.XUpperLimnmEditField3_2.Position = [1037 13 38 22];
            ...
            app.DViewPopOutButton.Tooltip = {'View the structure in a 3D figure.'};
            ...
            app.AutomaticButton.Tooltip = {'Set the axis limits of the figures to be automatically set.'};
            ...
            app.FixedButton.Tooltip = {'Allow the axis limits of the figures to be set manually.'};
            ...
            app.MicrotubuleDiameternmLabel.Tooltip = {'The diameter of each microtubule in nanometers.'};
            ...
            app.ComplexDiameternmSpinnerLabel = uilabel(app.StaticKinetUIFigure);
            ...
            app.ComplexDiameternmSpinnerLabel.Position = [17.5 803 133 22];
            app.ComplexDiameternmSpinnerLabel.Text = 'Complex Diameter (nm)';

            % Create ComplexDiameternmSpinner
            app.ComplexDiameternmSpinner = uispinner(app.StaticKinetUIFigure);
            ...
            app.ComplexDiameternmSpinner.ValueChangingFcn = createCallbackFcn(app, @MainParameterChange, true);
            app.ComplexDiameternmSpinner.Limits = [0 Inf];
            app.ComplexDiameternmSpinner.ValueChangedFcn = createCallbackFcn(app, @MainParameterChange, true);
            app.ComplexDiameternmSpinner.HorizontalAlignment = 'center';
            app.ComplexDiameternmSpinner.FontName = 'Arial';
            app.ComplexDiameternmSpinner.Position = [158 803 67 22];
            app.ComplexDiameternmSpinner.Value = 250;

            % Create NumberofMicrotubulesSpinnerLabel
            app.NumberofMicrotubulesSpinnerLabel = uilabel(app.StaticKinetUIFigure);
            ...
            app.LengthofArmnmSpinnerLabel.Position = [1 137 164 22];
            ...
            app.LengthofArmnmSpinner.Position = [172 137 64 22];
            ...
            app.NumberofArmsSpinnerLabel.Position = [1 179 166 22];
            ...
            app.NumberofArmsSpinner.ValueChangingFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.NumberofArmsSpinner.Limits = [0 Inf];
            app.NumberofArmsSpinner.RoundFractionalValues = 'on';
            app.NumberofArmsSpinner.ValueChangedFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.NumberofArmsSpinner.HorizontalAlignment = 'center';
            app.NumberofArmsSpinner.Position = [172 179 64 22];
            ...
            app.NumberofBoundArmsSpinnerLabel.Position = [-1 158 168 22];
            ...
            app.NumberofBoundArmsSpinner.ValueChangingFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.NumberofBoundArmsSpinner.Limits = [0 Inf];
            app.NumberofBoundArmsSpinner.RoundFractionalValues = 'on';
            app.NumberofBoundArmsSpinner.ValueChangedFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.NumberofBoundArmsSpinner.HorizontalAlignment = 'center';
            app.NumberofBoundArmsSpinner.Position = [172 158 64 22];
            ...
            app.PointMarkedSpinnerLabel.HorizontalAlignment = 'center';
            ...
            app.PointMarkedSpinnerLabel.Position = [1 116 166 22];
            ...
            app.PointMarkedSpinner.ValueChangingFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.PointMarkedSpinner.ValueChangedFcn = createCallbackFcn(app, @KinetochoreStructureValueChange, true);
            app.PointMarkedSpinner.HorizontalAlignment = 'center';
            app.PointMarkedSpinner.Position = [172 116 64 22];
            ...
            app.AnglefromMTAxisdegSpinnerLabel.HorizontalAlignment = 'center';
            app.AnglefromMTAxisdegSpinnerLabel.Tooltip = {'The angle of the vector that connects the center of the microtubule to the tapering point of the kinetochore complex relative to the microtubule''s central axis in degrees.'};
            app.AnglefromMTAxisdegSpinnerLabel.Position = [1 95 164 22];
            ...
            app.AnglefromMTAxisdegSpinner.Position = [172 95 64 22];
            ...
            app.AngleRangeforUnbounddegSpinnerLabel.Position = [38 70 179 22];
            ...
            app.LowerAngleSpinner.Position = [46 46 67 22];
            ...
            app.UpperAngleSpinner.Position = [140 46 67 22];
            ...
            app.UpperAngleLabel.Position = [117 47 21 22];
            ...
            app.KinetochoreColorChannel.Position = [709 501 66 22];
